...
|---|---:|---|
| Buffer d’extraction de classe | 1 MiB | `src/core/runtime.cpp:364` |
| Heap objet initial | 512 KiB | `src/core/runtime.h:175` |
| Arène des frames d’appel | 128 KiB | `src/core/interpreter.cpp:186` |
| Deux framebuffers 240×320 RGB565 | environ 300 KiB | `src/hal/display.cpp:35` et layer MIDP |
| Pile d’une fibre `Thread.start()` | 256 KiB | `src/cldc/natives.cpp:170` |
...
Le contrat de stockage du projet existe dans `src/kernel/drivers/storage/storage.h`, mais le backend SD/QSPI doit être implémenté si l’on quitte Linux. Sur Pi, `src/hal/file.cpp` peut rester le point d’accès `FILE*`, avec un chemin RMS contrôlé et sans appel shell fragile.
...
`src/hal/display.cpp:17-20` crée une fenêtre SDL de `2 × largeur` par `2 × hauteur`. Avec `JME_WIDTH=800` et `JME_HEIGHT=480`, la fenêtre demandée fait donc 1600×960, même si le framebuffer émulé fait 800×480.
...
Le projet attend les touches suivantes dans `src/hal/input.h:8-30` :
...
Le code actuel de `src/hal/input.cpp` ne traite encore que le clavier, la souris et les événements tactiles indirects. Il n’y a pas encore de backend `SDL_GameController`.
...
`src/kernel/kernel.h` n’est pas un kernel Linux. C’est une petite couche portable qui fournit :
...
Le projet peut conserver `src/hal/file.cpp` et les backends SDL pour une première version Pi. Le kernel projet devient alors une couche d’abstraction au-dessus des services Linux, plutôt qu’un noyau bare-metal.
