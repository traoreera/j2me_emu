...
namespace jvm
{
    ...
    namespace midp
    {
        ...
        namespace
        {
            ...
            void n_dc_createImage(NativeContext *ctx)
            {
                if (!ctx || !ctx->rt)
                    return;
                int w = argInt(ctx, 0);
            ...
            void n_sound_play(NativeContext *ctx)
            {
                // Nokia Sound.play(int) - no-op minimal implementation
                (void)ctx;
            }

            void n_sound_init_intlong(NativeContext *ctx)
            {
                // Nokia Sound.<init>(int, long) - no-op
                (void)ctx;
            }

            void n_sound_init_bytesint(NativeContext *ctx)
            {
                // Nokia Sound.<init>(byte[], int) - no-op
                (void)ctx;
            }
            void n_dg_drawImage(NativeContext *ctx)
            {
                if (!ctx || !ctx->rt)
                    return;
            ...
            jvm::Value native_Player_getControl(jvm::Runtime *rt, jvm::Interpreter *interp, jvm::Value *args, jvm::Value &res)
            {
...
void native_TimerTask_init(NativeContext *ctx)
{
    (void)ctx;
}
            jvm::Value native_VolumeControl_setLevel(jvm::Runtime *rt, jvm::Value *args, int argCount)
            {
                int level = args[1].i;
            ...
            SoundPlayer g_snd[kNumSound];

            SoundPlayer *sndSlotFor(Obj *p)
            {
            ...
            std::vector<uint8_t> sndDrainStream(Obj *stream, int cap)
            {
            ...
            void sndResample(std::vector<int16_t> &pcm, uint32_t srcRate)
            {
            ...
            bool sndParseWav(const std::vector<uint8_t> &raw, std::vector<int16_t> &pcm)
            {
                size_t len = raw.size();
            ...
            bool sndParseMidi(const std::vector<uint8_t> &raw, std::vector<int16_t> &pcm)
            {
            ...
            bool sndParseToneSeq(const std::vector<uint8_t> &raw, std::vector<audio::ToneEvt> &seq)
            {
                if (raw.size() < 2)
                    return false;
                if ((int8_t)raw[1] == 0)
            ...
            void mp_createPlayerIS(NativeContext *ctx)
            {
                ...
                if (getenv("JME_DEBUG") && stream && idx >= 0)
                {
            ...
            void mp_realize(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                if (p && p->cells && mediaStateOf(p) == 100)
                    p->cells[0].i = 200; // REALIZED
            }
            void mp_prefetch(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
            ...
            void mp_stop(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                if (!p || !p->cells)
                    return;
            ...
            void mp_deallocate(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
            ...
            void mp_close(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                if (p && p->cells)
                {
            ...
            void mp_setLoopCount(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                SoundPlayer *s = sndSlotFor(p);
                int v = argInt(ctx, 1);
            ...
            void mp_getControl(NativeContext *ctx)
            {
            ...
            void mp_getLong(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                SoundPlayer *s = sndSlotFor(p);
                if (s && s->kind == 1)
            ...
            void mp_getMediaTime(NativeContext *ctx)
            {
                Obj *p = ctx->thisObj;
                SoundPlayer *s = sndSlotFor(p);
            ...
            void mp_setMediaTime(NativeContext *ctx)
            {
            ...
            void vc_getLevel(NativeContext *ctx)
            {
                setInt(ctx, (int)lroundf(audio::masterVolume() * 100.0f));
            }
            void vc_setLevel(NativeContext *ctx)
            {
                int lv = argInt(ctx, 1);
                if (lv < 0)
                    lv = 0;
            ...
            void vc_getMute(NativeContext *ctx) { setInt(ctx, audio::masterVolume() <= 0.0f ? 1 : 0); }
            void vc_setMute(NativeContext *ctx)
            {
                int m = argInt(ctx, 1);
                audio::setMasterVolume(m ? 0.0f : 1.0f);
            }
            void mp_playTone(NativeContext *ctx)
            {
        ...
        void init(Runtime *rt, Interpreter *interp)
        {
            ...
            regClass(rt, "java/util/Timer", "java/lang/Object",
                     {{"schedule", "(Ljava/util/TimerTask;Ljava/lang/Runnable;J)V"}, {"scheduleAtFixedRate", "(Ljava/util/TimerTask;Ljava/lang/Runnable;JJ)V"}, {"cancel", "()V"}},
                     none);
