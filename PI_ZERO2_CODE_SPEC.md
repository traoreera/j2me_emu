...
Ce document décrit la cible logicielle du Raspberry Pi Zero 2 W. Il complète `HARDWARE_TARGET.md` en détaillant les changements nécessaires au build, à la mémoire, aux assets, au rendu et aux entrées.
...
| Croissance du heap | Auto-croissance dans `vm/runtime.cpp:53-90` | Empêcher les allocations non bornées sur Pi |
| Classes | Buffer statique de 1 MiB dans `vm/runtime.cpp:364` | Le garder hors du cache d’assets |
| JAR | Seek/read et buffers appelant dans `hal/jar_reader.h:48-76` | Ne pas slurper le JAR |
| Affichage | Framebuffer RGB565 et mise à jour SDL dans `hal/display.cpp:17-84` | Séparer taille logique et taille physique |
| Rendu MIDP | Opérations graphics dans `vm/midp_graphics.cpp` | Ajouter des chemins rapides et des statistiques |
| Entrées | Clavier, souris et tactile dans `hal/input.cpp:61-147` | Ajouter SDL GameController |
...
| Fichier | Travail |
|---|---|
| `launcher/launcher.h` | États, callbacks et configuration du menu |
| `launcher/launcher.cpp` | Découverte des JAR, manifeste, sélection et lancement |
| `hal/touch_overlay.h` | Définition des zones et de la configuration tactile |
| `hal/touch_overlay.cpp` | Dessin du joystick virtuel et conversion des événements |
| `hal/input.h` | État tactile et mode des contrôles virtuels |
| `hal/input.cpp` | Intégration du tactile physique, de l’overlay et du gamepad |
| `main.cpp` | Boucle d’états launcher/émulation et retour au menu |

## Configuration et fichiers à modifier

### Configuration

Les nouvelles options Pi doivent être documentées dans `README.md` et `AGENTS.md` :
...
| `main.cpp` | Parser les limites, choisir le profil, configurer la cadence et afficher le rapport mémoire |
| `vm/runtime.h` | Ajouter la limite maximale et les statistiques du heap |
| `vm/runtime.cpp` | Refuser la croissance au-delà du plafond |
| `hal/asset_cache.h` | Définir l’API du cache d’assets |
| `hal/asset_cache.cpp` | Implémenter l’index, l’éviction et les compteurs |
| `vm/render_cache.h` | Définir les sprites, tuiles et transformations réutilisables |
| `vm/render_cache.cpp` | Implémenter le cache avec une capacité fixe |
| `hal/display.h` | Séparer dimensions logiques, fenêtre et plein écran |
| `hal/display.cpp` | Appliquer scaling entier, letterbox et mise à jour SDL unique |
| `vm/midp_graphics.cpp` | Ajouter les chemins rapides, le clipping et les statistiques |
| `hal/input.h` | Exposer le mapping gamepad et les états associés |
| `hal/input.cpp` | Ajouter SDL GameController, hotplug et zone morte |
...
- la documentation `README.md` et `AGENTS.md` décrit les nouvelles options ;
- aucun changement non lié n’est inclus dans le même livrable.

## Références du dépôt

- `HARDWARE_TARGET.md` : choix de la carte, de l’écran, de l’audio et des commandes ;
- `PYTHON_RUST.md` : outillage externe optionnel ;
- `AGENTS.md` : build, tests et contraintes de portage ;
- `INTEGRATION.md` : contrats JAR, buffers et intégration ;
- `CMakeLists.txt` : sources et tests actuels ;
- `main.cpp` : configuration et boucle principale ;
- `vm/runtime.h` et `vm/runtime.cpp` : heap et chargement des classes ;
- `hal/jar_reader.h` : lecture JAR sans slurp ;
- `hal/display.cpp` : framebuffer et présentation SDL ;
- `hal/input.cpp` : état des entrées ;
- `vm/midp_graphics.cpp` : rendu MIDP ;
- `kernel/drivers/audio/` : mixer et backends audio.
